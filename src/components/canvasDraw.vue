<template>
  <div class="inner draw">
    ...
    <div class="canvas-color" :style="{'display': (showCanvasColor ?'block':'none')}">
      <ul>
        <li v-for="(item, index) in colors" :key="index" @click="setColor(item)"></li>
      </ul>
    </div>
    <!--画笔大小-->
    <div class="canvas-brush" :style="{'display': (showCanvasBrushs ?'block':'none')}">
      <ul>
        <li v-for="(pen, index) in brushs" :key="index" @click="setBrush(pen.lineWidth)"></li>
      </ul>
    </div>
    <!--橡皮擦大小-->
    <div class="canvas-eraser" :style="{'display': (showCanvasEraser ?'block':'none')}">
      <ul>
        <li v-for="(pen, index) in brushs" :key="index" @click="setEraser(pen.lineWidth)"></li>
      </ul>
    </div>
    ...
    <div class="wrap">
      <div class="canvasBox" ref="canvasBox">
        ...
        <canvas id="canvasBack" class="canvasBack" :width="canvasWidths" :height="canvasHeights" ref="backCanvas"></canvas>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      initCanvas() {
        ...
        this.drawPen(); //画笔功能
        canvasBack = this.$refs.backCanvas;
      ...
      drawPen: function() {
        ...
        document.onmousedown = function(ev) {
          ...
          // console.log('drawPen',ev,ev.target.id != 'canvas')
          if (ev.target.id != 'canvas' && ev.target.id != 'canvasBack') {
            return
          };
          ...
          document.onmousemove = function(ev) {
            ...
            if (ev.clientX <= offsetLeft) document.onmousemove = '';
            if (ev.clientY <= offsetTop) document.onmousemove = '';
            // if (!onOff) return;
            // onOff = false;
            // setTimeout(function () {
            //   onOff = true;
      ...
      eraser: function() {
        ...
        document.onmousedown = function(ev) {
          ...
          document.onmousemove = function(ev) {
            ...
            //直接绘制
            that.context.beginPath();
            that.context.clearRect(ex - (that.config.eraserWidth / 2), ey - (that.config.eraserWidth / 2), that.config
              .eraserWidth, that.config.eraserWidth);
            that.context.closePath();
            that.context.fill();
          };
          document.onmouseup = function() {
      ...
      drawRect: function() {
        ...
        document.onmousedown = function(ev) {
          ...
          let offsetLeft = ev.target.parentNode.offsetLeft;
          let offsetTop = ev.target.parentNode.offsetTop;
          if (ev.clientY < canvas.offsetTop) {
            return;
          }
      ...
      render() {
        var that = this;
        this.contextBack.clearRect(0, 0, this.canvasWidths, this.canvasHeights);
        for (var i = 0; i < that.DATA.length; i++) {
          ...
          switch (that.DATA[i].attr) {
            ...
            case 'clear-rect':
              ...
              //矩形按下列规则绘制
            case 'rect':
              that.contextBack.fillStyle = that.DATA[i].c;
              that.contextBack.beginPath();
              that.contextBack.fillRect(that.DATA[i].x, that.DATA[i].y, that.DATA[i].w, that.DATA[i].h);
              that.contextBack.closePath();
              that.contextBack.fill();
              break;
              //画笔、直线按下列规则绘制
            case 'point-line':
              // case 'line':
      ...
      controlCanvas(action) {
        switch (action) {
          ...
          case "clear": //清除画布
            //清空画板
            this.contextBack.clearRect(0, 0, this.context.canvas.width, this.context.canvas.height);
...
<style lang="less" scoped>
  ...
  .inner {
    ...
    .wrap {
      ...
      // top:220/@rem;left:50%;transform: translateX(-50%);position:relative;
      pointer-events: auto;
